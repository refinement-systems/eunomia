...
//! through (rev2§3.7) — and the future IDL is just a second `Codec` impl. Bodies are
...
pub enum WireError {
    ...
    Body,
    /// Bytes remained after the declared body — a framing violation (rev2§3.7).
    Trailing,
}

/// The serializer seam (rev2§3.7): module-private so it can be swapped for an IDL
...
impl Codec for Postcard {
    ...
    fn decode_body<B: DeserializeOwned>(bytes: &[u8]) -> Result<B, WireError> {
        ...
        // junk inside the declared body region — reject it (rev2§3.7).
