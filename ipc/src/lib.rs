//! Userspace IPC crate — shared by every server (spec rev0§3.5, rev0§3.7).
//!
//! Responsibilities:
...
//!   - Lost-wakeup discipline around the notification object (rev0§3.6)
//!
...
//! kernel wait-set object when that lands (spec rev0§3.6).
...
// alloc rides with the `wire` feature: the wire codec (rev0§3.7) (de)serializes
