...
//!
//! The fixed message header (spec §3.7): `decode` is total over all byte strings
//! and rejects any non-`HEADER_SIZE` length (short input and trailing bytes
//! alike), and `encode`/`decode` are mutually inverse — the bijection that lets
//! the header layer stay byte-stable while every layer above it migrates.
//!
