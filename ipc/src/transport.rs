...
//! word are all the *kernel's* behavior (rev2§3.3, rev2§3.4, rev2§3.6); this trait only
...
/// The channel events a binding can target (rev2§3.3, rev2§3.6).
...
/// `send_nb` failure (rev2§3.3): `Full` is backpressure (retry when writable), not
...
/// `recv_nb` failure (rev2§3.3): `Empty` (wait for readable), `NoSlot` (receiver
/// has no free cspace slot — the message stays queued, make room and retry,
/// rev2§3.4), `Closed` (peer-closed).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum RecvErr {
    Empty,
    NoSlot,
...
/// A successful receive: payload length and the cap-present mask (rev2§3.3).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct RecvOk {
    pub len: usize,
    pub cap_mask: u64,
...
/// The kernel IPC surface the reactor needs, 1:1 with `sys.rs` / rev2§3.3, rev2§3.6.
pub trait Transport {
    /// Non-blocking send (rev2§3.3): `Full` when the queue is full, never a drop.
    fn send_nb(&self, ch: Chan, data: &[u8], caps: Option<&[u32; 4]>) -> Result<(), SendErr>;

    /// Non-blocking receive (rev2§3.3): caps land in `dests`; `Empty`/`NoSlot` per
    /// `RecvErr`. `buf` must hold a full inline payload (256 bytes, rev2§3.1).
    fn recv_nb(
        &self,
        ch: Chan,
        buf: &mut [u8],
    ...
    /// Bind a channel event to `(notif, bits)` (rev2§3.6). Persistent: a later
    /// event ORs `bits` into the notification word.
    fn bind(&self, ch: Chan, ev: Event, notif: Notif, bits: u64) -> Result<(), i64>;

    /// OR `bits` into a notification word, waking a waiter or accumulating (rev2§3.6).
    fn notif_signal(&self, n: Notif, bits: u64);

    /// Consume the accumulated word if non-zero, else block (rev2§3.6). Returns the
