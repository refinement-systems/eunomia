...
pub enum ObjType {
    ...
    Frame,
    /// param = table-pool pages (pool-at-creation, rev2§2.5).
    Aspace,
    /// A sub-range untyped (rev2§2.3: untyped derivations are page-aligned
    ...
    /// parent funds for one child (rev2§5.1).
    Untyped,
}
...
///
/// The rev2§2.5 rights-inheritance table is proven as theorems: a Frame inherits the
        ...
        )),
{
    ...
    proof {
        // rev2§2.5 sub-untyped-never-PHYS: masking to READ|WRITE clears the PHYS bit for
...
/// reclaim primitive (rev2§2.5: "reclaiming the range is revoke(untyped) then
/// watermark reset"). A parent reuses one child-sized donation across many
/// spawns this way (rev2§5.1); the next retype re-zeroes the frames it carves.
///
