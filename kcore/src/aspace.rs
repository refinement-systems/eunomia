...
//!
//! Mapping state lives in the frame cap, not here (rev2§2.5): one mapping per cap
...
pub enum MapError {
    BadVa,
    AlreadyMapped,
    /// Table pool exhausted — donate a bigger pool (rev2§2.5: one error path).
    NeedMemory,
}
...
/// `PERM_DEVICE` is set (rev2§2.5). The output address is masked to bits [47:12].
///
/// Verified — the rev2§2.5 **isolation theorem**, ∀ `(pa, perms)`: AF + PXN
...
/// (rev2§2.5 "accepts top-ups"). Every live descriptor targets an index
