...
impl Channel {
    ...
    unsafe fn slot(this: *mut Channel, ring: usize, i: u32) -> *mut MsgSlot {
        let base = this.add(1) as *mut MsgSlot;
...
pub unsafe fn endpoint_cap_dropped(ch: *mut Channel, end: ChanEnd) {
    let e = end_idx(end);
    (*ch).end_caps[e] -= 1;
    if (*ch).end_caps[e] == 0 {
        fire(ch, 1 - e, EV_PEER_CLOSED);
    }
}

unsafe fn fire(ch: *mut Channel, end: usize, event: usize) {
    let b = (*ch).bindings[end][event];
    if !b.notif.is_null() {
        notification::signal(b.notif, b.bits);
    }
}
...
pub unsafe fn destroy_channel(ch: *mut Channel) {
    for ring in 0..2 {
        let depth = (*ch).depth;
        for i in 0..depth {
            let slot = Channel::slot(ch, ring, i);
