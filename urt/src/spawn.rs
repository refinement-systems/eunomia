//! The rev0§5.1 canonical parent loop, factored out of any one parent.
//!
...
//! inherits them:
//!
//!   * **bind before start, wait, then reap** (rev0§5.1). The on-exit/on-fault
...
//! (the runtime exit path, rev0§5.1) — so `Exit::Exited(STATUS_PANIC)` is a
//! crash, distinct from any status a child passes deliberately.

use ipc::sys;

/// Why a child stopped, read out of its terminal report record (rev0§5.1).
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum Exit {
    /// Voluntary `thread_exit(status)`.
    Exited(u64),
    /// Unhandled fault — suspended, not destroyed (rev0§5.3). `esr` is the raw
...
pub struct SpawnRec {
    ...
    /// before it even reads the report — the rev0§3.6 bit-group scan in
    /// miniature.
    pub fault_bit: u64,
}
...
impl SpawnRec {
    ...
    /// empty on return). Call before starting the thread (rev0§5.1).
    ///
    ...
    pub fn arm(&self, event_notif: u32, scratch: u32) -> i64 {
        ...
        {
            // Duplicate first (rev0§3.4): the bind below moves the cap out of
    ...
    /// (rev0§5.1). Doing both here, in this order, is exactly so a parent can't
