//! The §5.1 canonical parent loop, factored out of any one parent.
//!
...
//! inherits them:
//!
//!   * **bind before start, wait, then reap** (§5.1). The on-exit/on-fault
...
/// Why a child stopped, read out of its terminal report record (§5.1).
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum Exit {
    /// Voluntary `thread_exit(status)`.
    Exited(u64),
    /// Unhandled fault — suspended, not destroyed (§5.3). `esr` is the raw
...
pub struct SpawnRec {
    ...
    /// before it even reads the report — the §3.6 bit-group scan in
    /// miniature.
    pub fault_bit: u64,
}
...
impl SpawnRec {
    ...
    /// empty on return). Call before starting the thread (§5.1).
    ///
    ...
    pub fn arm(&self, event_notif: u32, scratch: u32) -> i64 {
        ...
        {
            // Duplicate first (§3.4): the bind below moves the cap out of
    ...
    /// (§5.1). Doing both here, in this order, is exactly so a parent can't
