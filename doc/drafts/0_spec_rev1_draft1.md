# Eunomia OS — Design Document (rev1)
...
- **(c) Cap-to-page-table correspondence, mapping, and clearing.** The cap-side map and unmap are both proven over object state — a derived copy starts unmapped, mapping records the entry's coordinates in the cap, and deletion drives an unmap at those coordinates — and the real writing and clearing of page-table entries is proven separately over raw page-table memory; the join — that the cap's recorded mapping is the true entry location and that map and unmap truly write and clear it — lives in the unverified kernel store (§2.5).
- **(d) Thread-lifecycle shell.** The kernel core mechanizes the report record's mutation (first-write-wins, absorbing), its sole mutator, its survival of object destruction, and the spawn-time priority-ceiling gate — the refusal to start a thread whose requested priority exceeds its thread cap's ceiling (§5.4). It does not mechanize the "suspended, never rescheduled" state (exception entry, syscall exit, scheduler), the anti-forgery and anti-suppression access control (the rights gates plus the spawn-time cap-distribution convention), or the exit/read-report syscall dispatch and register marshalling (§5.1, §5.3).
- **(e) Storage-recovery content coverage.** The verified surface over the WAL and superblock covers the in-bounds, totality, and maximal-run structural properties; per-record content acceptance is uninterpreted and the commit routine is plain Rust, so the full replay-equality invariant is mechanized nowhere on the real code and remains the TLA+ model's alone — where the recovery-step action property (§6) checks it.
