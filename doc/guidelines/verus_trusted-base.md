...
(`check_capacity`, rev2§4.5) is Verus-mechanized: its pure arithmetic is the free
...
per-step FIFO discipline (`kcore/src/channel.rs`, rev2§3.3) is Verus-mechanized under
...
`IpcReactor` model owns those global arms; only the local per-step refinement is
Verus-mechanized. No trusted seam added (the tally stays 14) and no Baseline change: the
FIFO-label `spec fn`s are non-recursive and carry no proof obligation, so kcore's count
(Baseline row below) is unchanged by this note.
...
skips a cleared slot, never freed memory — is Verus-mechanized as
...
revoke walk's `EventuallyRevoked` liveness — the TLA `CapRevocation` model owns that
global arm and stays the liveness oracle. No trusted seam added (the tally stays 14); the
two new `proof fn`s are counted in kcore's Baseline (the `fire_safe` `spec fn` is
non-recursive, +0), below.
...
local WAL-byte/queue projection is Verus-mechanized — the WAL-projection routing note below),
...
`CommitProtocol` model owns those global invariants and stays the design oracle. No trusted
seam added (the tally stays 14) and no Baseline change from these two labels: they are
inline clause comments on already-verified `ensures`, carrying no proof obligation (the
WAL-projection below is what adds obligations to cas's count, Baseline row).
...
`CommitProtocol` model owns that global arm. No trusted seam added (the tally stays
14, the lifetime-join row already exists); the corollary + teeth `proof fn`s are counted in
cas's Baseline (the `recover_reconstructs` `spec fn` is non-recursive, +0; the
...
off-target tests, rather than copying the asm. The thin
...
Three further `eunomia-sys` modules are **plain Rust
...
The mandatory `GlobalAlloc` arm is **trusted shell over a
separately-verified algorithm — no `verus!{}` obligation, no new seam, tally stays
14**. `pal` holds a process-global `static HEAP: urt::Heap<N>` and two more
...
only if and when the mechanism lands. `eunomia-sys` takes `urt` as a **target-gated**
dependency (the userspace cross-build only), so the host `-p eunomia-sys` verify graph
excludes it — the heap **algorithm** verifies under its own `-p urt` / `-p freelist` gates
(30 / 30), not this crate's session; eunomia-sys's own count is **16**
(`heap.rs`/the `pal` shims add no `verus!{}`).

The bring-up `stdio` arm and the process exit/abort terminus are
**trusted shell over a host-tested seam — no `verus!{}` obligation, no new seam,
...
against the kernel literal). `pal` carries one more `#[no_mangle]` shim
(`__eunomia_stdio_write`) delegating to it. The vendored PAL arms — `sys/stdio/eunomia.rs`
(stdout/stderr → debug-log, stdin EOF until the rev2§5.1 console, `panic_output` → debug-log),
...
rev2§7 / C-M9 pre-console scaffold, retained only for kernel-internal panic last-words now
that the userspace console channel (rev2§5.1) carries user-facing stdio.

The `sys/time` arm (`Instant` + `SystemTime`) is **trusted
...
tick→ns conversion is verified total + monotone — the urt time row, 30/0); `Instant` reads
`urt::time::now_mono_ns`, which reuses that *same* verified conversion over a
zero wall/counter base to get monotonic ns straight from CNTVCT/CNTFRQ — so it needs no
`"time"` grant (a process measuring durations without one still works; the futex timeouts
depend on this). `now_mono_ns` is a non-`verus!{}` exec fn, `aarch64`-gated out of the
Verus host build, so it adds no urt obligation (Baseline row). `pal` carries two more `#[no_mangle]` shims
(`__eunomia_mono_ns`/`__eunomia_wall_ns`) delegating term-for-term, and `bootstrap::init`
holds a target-gated `attach_grants` that calls `urt::time::attach` when the startup block
...
**TPIDR_EL0 TLS save/restore routing note.** The AArch64
`TPIDR_EL0` register (the EL0 thread pointer / TLS base, RW at EL0) survives a context
switch, the threading prerequisite for per-thread TLS. `kcore::thread::TrapFrame` carries
a `tpidr` field (288 bytes total, a trailing pad word preserving the 16-byte SP
...
unchanged**. The hand-coded asm byte offsets are pinned to the struct by a
...
of saved EL0 register state, covering the `tpidr` slot as well, so it adds **no `external_body` and
...
**Heap-spinlock + per-thread-TLS routing note.** In-process
`std::thread` rests on three trusted, non-Verus constructs, all folding under
...
- **The `urt::Heap` yielding spinlock (`urt/src/lock.rs`)** serializes the free-list
  ...
  Same tier and category as the futex bucket lock (which reuses this primitive). Its
...
- **The per-thread `TPIDR_EL0` TLS block (`eunomia-sys/src/tls.rs` `msr`/`mrs` + the
  `sys/thread` trampoline)** — the per-thread pointer block set up in `_start` (main) and
  the thread trampoline (spawned), which std's `local_pointer!` current-thread/id storage
  rides. This is the userspace mirror of the trusted TLS-register marshalling; it **extends
  the TPIDR_EL0 routing note above** (the thread-lifecycle asm shell, §6.1(d)) to the
  userspace block setup. On-target witnesses: the QEMU spawn smoke + its
  distinct-per-thread-TLS-id check. *(The verified `urt::tls` key table indexes this block
  and the exit-time destructor runner reclaims it — see the TLS-key-table routing note
  below.)*
- **The `ThreadStartAs` `x6` arg stays verified, not trusted:** the seven-register
  `kcore::sysabi::decode` (`[u64;7]`) and `eunomia-sys::encode` prove it, adding no
  `external_body` (both counts live in the Baseline rows below).

**Futex-backend routing note.** The userspace `sys::futex` backend
— the one primitive that lights the whole upstream lock stack
(`Mutex`/`Condvar`/`RwLock`/`Once`/`Parker`) — rests on trusted, non-Verus constructs, all
...
- **The address→waiter dispatch over notifications (`urt/src/futex.rs`)** — a
  process-global bucket table (keyed by the futex address, guarded by the *reused* heap
  spinlock) whose `futex_wait` enqueues under the lock (word-checked before
  ...
  ghost-atomic pin, `doc/guidelines/verification.md`) — same tier and category as the
...
**Entropy-seed routing note.** The per-process entropy DRBG
...
- **The seed *decode* is verified surface, counted on the loader row.** The
  ...
  clause, and is counted in the loader Baseline count (**30**). Fuzz + Miri corpus + the
  `startup2_truncated_seed_refused` regression are its companion oracle.
- **The DRBG shuffle (`urt/src/random.rs`) is host-tested plain Rust under two `verus.md`
  §11 categories, not a new seam.** The xoshiro256\*\* generator (`Drbg::next_u64`, the
  `expand_seed`/`fresh_seed` splitmix-style sub-seeding) folds under **§11 category (2),
  out-of-scope total function** — trusted for *totality + determinism only*, no deeper
  property: its randomness *quality* is explicitly **not** a verification property
  (rev2§5.1 MVP, a documented-predictable non-cryptographic seed — QEMU `virt` has no
  entropy source), and its structural invariants are *sampled*, not theorems — "`fresh_seed`
  never returns the raw seed" is a fixed-point property not excluded ∀ states over the
  2²⁵⁶ state space, and sub-seed distinctness is statistical. The no-seed policy folds
  under **§11 category (3), runtime-only guard**: `fill_bytes`/`fresh_seed` call
  `no_seed_abort()` (a loud abort) when unseeded — the `urt::time::now_utc_ns` precedent,
  the §11 inverse-leak boundary. The `unsafe impl Sync` over an `UnsafeCell<Option<Drbg>>`
  is the exact `urt::Heap` posture — **mutual exclusion by the reused, Loom-certified
  `lock::SpinLock`** (no wait/wake, so no new interleaving model). Kept honest by the same
  Miri + proptest tier as the heap arena byte-region: the `urt/src/random.rs` `mod tests`
  cases `deterministic_stream_from_a_fixed_seed`, `distinct_seeds_diverge`,
  ...
  `fill_locked_aborts_when_unseeded` (the no-seed abort), and `global_seed_then_fill`.
- **`Drbg::fill`'s little-endian word serialization is verified surface, counted on the
  urt row (30).** The `verus!{}` helper `urt::random::u64_to_le(w) -> [u8; 8]` (proven
  `r@ == le_bytes::u64_le(w)`) is what `fill` calls for each generated word, so the bytes
  it emits are exactly the little-endian split of the DRBG's `u64` stream — the one
  genuinely-provable piece, lifted onto the shared `le_bytes::u64_le` spec (urt takes the
  no_std `le-bytes` path-dep for it). Mechanized, then, are the seed *decode* (loader row)
  and this *fill* serialization (urt row); the xoshiro transition and every quality
  property stay trust-routed under §11 (2)+(3) above.
...
**TLS-key-table routing note.** Real `thread_local!` macro storage +
destructors run eunomia on std's key-based TLS (`sys/thread_local/os.rs`,
the motor shape). The **key allocation is verified surface — the only new obligations —
and the tally stays 14**:

- **The `urt::tls::KeyTable` is verified surface, counted on the urt row (Baseline row).** A
  thin `KeyTable` over the already-verified `slots::SlotAlloc<1>`: `create` hands out an
  in-range key that was free and is now live (so keys are distinct — the key-uniqueness
  the TLS layer stands on, *proven*), exhaustion is exact, `destroy` frees a live key (a
  double-destroy a contract-checked impossibility). Both `local_pointer!` (CURRENT/ID) and
  every `thread_local!` draw keys here — a verified key table, so key-uniqueness is
  *proven* rather than assumed of a raw counter. The per-key destructor registry
  (`[Option<Dtor>; TLS_KEYS]`) is plain sibling bookkeeping (a table of opaque function
  pointers is not a verifiable property), guarded by the reused Loom-certified
  `lock::SpinLock` — the `urt::random` posture, **no new interleaving model**; Miri is the
  data-race oracle (`urt::tls::` tests in the `-p urt` sweep).
- **The seam get/set/run/free (`eunomia-sys/src/tls.rs`) stay trusted asm shell.** Per-key
  *values* live in each thread's `TPIDR_EL0` block, so `get`/`set` are the `mrs`-based
  read/write of `[TPIDR + key]` (each thread touches only its own block, so lock-free);
  `run_thread_dtors` iterates live keys at thread exit and `free_thread_block` reclaims a
  spawned thread's heap block at exit. This is the userspace TLS-register
  marshalling — it **extends the TPIDR_EL0 routing note above** (thread-lifecycle asm
  shell, §6.1(d)), not a new seam.
- **The vendored-std `os.rs` storage + `guard` + the `key/eunomia.rs` bridge + the
  `__eunomia_tls_*` shims are upstream/thin.** eunomia falls into upstream's `os` storage and
  the "std is the only runtime" `guard::enable` (`{}`), because it drives destructors itself
  at the trampoline / `_start` exit (`run_dtors` → `rt::thread_cleanup` → `free`). The
  `key/eunomia.rs` bridge and the `__eunomia_tls_{create,get,set,destroy,run_dtors,
  free_thread}` shims are term-for-term delegation, holding no logic. §11 host test: `cargo
  test -p urt` + the QEMU `thread_local!`-destructor / per-thread-storage smoke (`STD35 PASS`).

**Path-resolver routing note.** The storaged fs client's one piece of
untrusted byte-parsing — turning a raw `/`-separated `OsStr` path into the wire
component list — is **verified surface, and the tally
stays 14**:

- **`eunomia_sys::path::resolve` (`eunomia-sys/src/path.rs`) is verified surface,
  counted on the `-p eunomia-sys` row (16).** Mechanized **total** over every
  `&[u8]` (no panic / no out-of-bounds read), and every accepted component satisfies the
  same predicate storaged's `cas::prolly::validate_name` enforces (1..=255 bytes, no NUL,
  no `/`, and — resolved, never stored — not `.`/`..`, rev2§4.9) and is a subrange of the
  input. So no accepted path can be a `..`, and prepending the result to the handle's
  subtree (the server's `full_path`) can never escape it — the rev2§2.3 "confinement by
  unreachability" made machine-checked (a `..` at depth 0 is *denied* with `None`, not
  clamped). It is **no-alloc** (borrowed subranges into a fixed `[&[u8]; MAX_COMPONENTS]`
  arena — the `loader::startup::decode` shape), so the host `-p eunomia-sys` verify graph
  is byte-identical (no `alloc`/`Vec` obligations enter; `vstd` stays `default-features =
  false`). Pure slice/byte reasoning citing only `vstd` — **no `external_body`, no new
  seam**.
- **The resolution *semantics* are a lossy transform, so test-routed, not a Verus
  property** (the `verus.md` §8 lossy-decode tier): that `.`/`..` resolve *correctly*
  (`a/../b → [b]`) is carried by a **cargo-fuzz differential oracle** vs a plain reference
  resolver (`eunomia-sys/fuzz/fuzz_targets/path.rs`), a **host proptest** (differential +
  join↔resolve idempotence, `cargo test -p eunomia-sys`), the **Miri** corpus replay +
  regression reproducers (`eunomia-sys/tests/fuzz_{corpus,regressions}.rs`, joining the
  `-p eunomia-sys` sweep), and the QEMU end-to-end run — the same tool tier as the
  `loader::startup` decoder.
- **The `eunomia-sys/src/fs.rs` marshalling stays the trusted shell** (the `sys/stdio`
  posture): `resolve_path` is a thin `alloc` adapter that copies the verified resolver's
  borrowed components into the `Vec<Vec<u8>>` wire path and maps a rejection to
  `ERR_FS_BAD_PATH`, holding no byte logic of its own. §11 host test: `cargo test -p
  eunomia-sys` + `cargo +nightly fuzz run path` + the QEMU fs smoke (`STD4 PASS`, which
  now exercises `docs/./smoke`/`docs/../docs/smoke` resolution and an escaping-`..`
  refusal at EL0 over storaged).
...
| `usize::checked_next_multiple_of` | `kcore/src/untyped.rs:274` | vstd has no spec yet; the Untyped arm needs only that it returns an `Option`, then re-checks positivity. | positivity re-checked at the call site; signature-only trust. |
| `CapSlot::empty` | `kcore/src/cspace.rs:1849` (`assume_specification`; the `const fn empty` it specs is at `:177`) | plain-Rust `const fn` shared with the kernel shell; the `ensures` state what it builds (empty cap, all four CDT links `None`) so `slot_move`'s final clear verifies. | consumed by the verified `slot_move`; `ensures` pins the construction. |

### (3) Runtime-only guard

| Construct | Location | Reason | Host test |
|---|---|---|---|
| `debug_check_free` | `urt/src/slots.rs:362` | a `debug_assert!` double-free guard; `external_body` so Verus doesn't see the `panic!` lowering (forbidden in exec). The *static* guarantee is `SlotAlloc::free`'s `!is_free_spec` precondition. | `double_free_panics` (urt host test) pins the runtime witness. |

### (4) Opaque layout fact — size positivity

| Construct | Location | Reason | Host test |
|---|---|---|---|
| `ExTcb` | `kcore/src/untyped.rs:260` | `external_type_specification` registering `Tcb` opaque so `size_of` typechecks in the verified `carve`. | `object_size_positive` (`kcore/src/untyped.rs:829`). |
| `ExNotifObj` | `kcore/src/untyped.rs:265` | opaque registration of `NotifObj`. | `object_size_positive`. |
| `ExTimerObj` | `kcore/src/untyped.rs:270` | opaque registration of `TimerObj`. | `object_size_positive`. |
| `fixed_object_bytes` | `kcore/src/untyped.rs:287` | `ensures r > 0`; Verus can't derive `size_of::<Tcb>() > 0` for the opaque types above, so this names the size-positivity fact. | `object_size_positive`. |
| `CSpaceObj::bytes_for` | `kcore/src/untyped.rs:237` | `ensures r > 0`; the per-object size helper lives in plain Rust (shared with the shell); `carve`'s geometry needs only positivity. | `bytes_for_positive` (`kcore/src/untyped.rs:813`). |
| `Channel::bytes_for` | `kcore/src/untyped.rs:242` | `ensures r > 0`; as above. | `bytes_for_positive`. |
| `AspaceObj::bytes_for` | `kcore/src/untyped.rs:247` | `ensures r > 0`; as above. | `bytes_for_positive`. |
...
> `CapSlot::empty` (`cspace.rs:1849`). Ground truth is **6** `assume_specification`
> statements, recorded above.

> **The 10 transparent cspace `external_type_specification` registrations are not seams
> and add 0 to the tally.** `kcore/src/cspace.rs:278-331` defines 10
...
> in `kcore/src/untyped.rs:257-270`), none carry `external_body` and none introduce a
...
| shared `FreeList` (free-list allocator core + `is_full`/`is_allocated` guard accessors, in the `freelist` crate) | `cargo verus verify -p freelist` | 30 verified, 0 errors (the no_std/no-alloc gate. The count includes the `two_allocs_disjoint_teeth` driver, which performs two real carves and threads `alloc`'s coverage `ensures` into `lemma_two_allocs_disjoint`, discharging that disjointness lemma's premises from code rather than a comment. The three `spinoff_prover` merge proofs carry `rlimit(1)` (`free_insert`, `free_replace`, `free_both`) — sized for the worst re-verification context across the standalone no-alloc gate and the `vstd[alloc]` prelude context (when `freelist` is re-verified as a transitive dep of an `alloc` crate like `virtio-blk`). After trigger reductions each proof's consumption is well under 400k in both contexts (worst-context per function: `free_insert` ~358k no-alloc, `free_replace` ~259k alloc, `free_both` ~183k no-alloc), all safely below the 1M ceiling. The `rlimit` is a solver ceiling, not a cost, so it does not affect the no-alloc gate's verified count) |
...
| urt slots + time + heap + tls | `cargo verus verify -p urt` | **30 verified, 0 errors** — urt's *own* surface: the slot bitmap, `utc_ns_at`, the TLS `KeyTable` (`new`/`create`/`destroy` over the slot bitmap — `create` rlimit 8391, `destroy` 5035, `new` 2709), and `random::u64_to_le` (the DRBG's little-endian word serialization, proven `r@ == le_bytes::u64_le(w)`, rlimit 13775). urt takes the no_std `le-bytes` path-dep for that shared `u64_le` spec, so a cold `-p urt` re-verifies its gated deps (ipc 71, freelist 30, le-bytes 6). The heap allocator's *algorithm* is the `freelist` dep (**30/0**); the heap *wrapper* is a plain-Rust arena seam (`UnsafeCell<[u8; N]>` + `base.add(off)`), **0 obligations**, kept honest by the Miri+proptest tier (`cargo +nightly miri test -p urt`). Disclosed MVP bounds in that wrapper (test-routed, not Verus-mechanized): `HEAP_RANGES = 1024` fragmentation cap, `MAX_ALIGN = 64`, `dealloc`-at-cap → safe leak (never aborts a free) — see `urt/src/lib.rs` module doc. |
