...
- **The per-thread `TPIDR_EL0` TLS block (`eunomia-sys/src/tls.rs` `msr`/`mrs` + the
  ...
  distinct-per-thread-TLS-id check. *(Std-port 3.5 replaces the raw slot counter that
  indexed this block with the verified `urt::tls` key table and adds the exit-time
  destructor runner — see the TLS-key-table routing note below.)*
...
**TLS-key-table routing note (std-port 3.5).** Real `thread_local!` macro storage +
destructors (findings #12) move eunomia onto std's key-based TLS (`sys/thread_local/os.rs`,
the motor shape). The **key allocation is verified surface — the only new obligations —
and the tally stays 14**:

- **The `urt::tls::KeyTable` is verified surface, counted on the urt row (`25 → 29`).** A
  ...
  double-destroy a contract-checked impossibility). It **replaces std-port 3.2's raw
  `NEXT_SLOT` atomic counter** — both `local_pointer!` (CURRENT/ID) and every
  `thread_local!` now draw keys here. The per-key destructor registry
...
- **The seam get/set/run/free (`eunomia-sys/src/tls.rs`) stay trusted asm shell.** Per-key
  *values* live in each thread's `TPIDR_EL0` block, so `get`/`set` are the `mrs`-based
  ...
  spawned thread's heap block (fixing the 3.2 block leak). This is the userspace TLS-register
  marshalling — it **extends the 3.1/3.2 TPIDR_EL0 routing note** (thread-lifecycle asm
...
**Path-resolver routing note (std-port 4.2).** The storaged fs client's one piece of
untrusted byte-parsing — turning a raw `/`-separated `OsStr` path into the wire
component list — becomes **verified surface, the only new obligations, and the tally
stays 14**:

- **`eunomia_sys::path::resolve` (`eunomia-sys/src/path.rs`) is verified surface,
  counted on the `-p eunomia-sys` row (`7 → 16`).** Mechanized **total** over every
...
| urt slots + time + heap + tls | `cargo verus verify -p urt` | **29 verified, 0 errors** — urt's *own* surface (slot bitmap + `utc_ns_at` + the std-port 3.5 TLS `KeyTable`, which added `new`/`create`/`destroy` over the slot bitmap, `25 → 29`, cheap: `create` rlimit 8391, `destroy` 5035, `new` 2709, every pre-existing `slots`/`time` obligation byte-identical). The heap allocator's *algorithm* is the `freelist` dep it re-checks transitively (**30/0**); the heap *wrapper* is a plain-Rust arena seam (`UnsafeCell<[u8; N]>` + `base.add(off)`), **0 obligations**, kept honest by the Miri+proptest tier (`cargo +nightly miri test -p urt`). Disclosed MVP bounds in that wrapper (test-routed, not Verus-mechanized): `HEAP_RANGES = 1024` fragmentation cap, `MAX_ALIGN = 64`, `dealloc`-at-cap → safe leak (never aborts a free) — see `urt/src/lib.rs` module doc. |
