...
**Wall-clock time** uses a one-shot boot read, not a driver: init (which holds all device caps) reads the PL031 RTC once at boot and publishes `(seq, wall_base, cntvct_base, cntfrq)` in a read-only shared frame mapped into every process — a **time page**, vDSO-style. `wall_base` is pinned as **UTC nanoseconds since the Unix epoch, signed 64-bit**; wall time = `wall_base + (CNTVCT − cntvct_base) · 10⁹ / cntfrq`, computable by anyone with zero syscalls and zero IPC, and the same representation is what snapshot rows store (§4.7). `seq` is a sequence word, today constant — the page is write-once at boot — one integer spent now (§4.2's birth-generation move) so that deferred clock setting and drift correction (§8) become a seqlock discipline rather than a flag-day retrofit of a page mapped everywhere. Pre-time-page images, whose snapshot rows hold raw CNTVCT ticks, are not migrated: they are re-created with `mkfs` — an experimental system owes no migration here, recorded so it is a stance rather than a surprise. All stored time is UTC; timezones are presentation, owned by the shell. Boot-relative timestamps were rejected once retention policies existed: "older than 30 days" must order across reboots.
...
- **Timestamp:** server-assigned at snapshot time, stored as UTC nanoseconds (§2.6); client-supplied times are not accepted. (Mechanism: the time page, §2.6.)
