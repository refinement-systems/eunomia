# Eunomia OS — Design Document (rev3, **DRAFT**)

> **Draft status.** This is the rev3 *draft*, derived from `2_spec_rev2.md`. rev2 already
> folded in every **fix-now** and **follow-on** finding from the drift audit
> (`doc/results/69_verus-findings.md`) — the revoke contract split (D-A1/A2/A3), the
> cap-carried monotone priority ceiling (D-B1), the receive-half move export (D-D1), the
> leaf-op refcount exports (D-E1/E2/F2), and the §6 table rewrite (D-H1/H2) — all verified and
> annotated inline. This draft adds what rev2 still lacked: the **accepted-documented** honest
> proof-boundary notes that doc 69's "Inputs to 9e" prescribed but that were never applied
> (D-B2, D-C1, D-C2, D-C3, D-D2, D-E3, D-E4, D-F1, D-F3, D-F4 — D-G1 already landed in §6
> row (a)), consolidated into a new **§6.1 verified-vs-trusted** boundary statement.
>
> Three of those findings are not boundaries but **genuine design ambiguities** the spec left
> open and the implementation silently resolved. They are surfaced as numbered **open
> questions (OQ-1…OQ-3)** in **§11**, each with the current code reading flagged as a
> *proposed* resolution and an alternative — **none ratified yet**. Inline markers
> ("*Draft decision OQ-N — §11*") point to them. These are the decisions to settle when rev3
> leaves draft; everything else in this document is settled rev2 text or a faithful
> description of what the verified code already does.
...
**Kernel caps (memory, threads, channels, cspaces, IRQs): seL4-style capability derivation tree (CDT).** Every copy, mint, or derivation records a parent–child edge, threaded through the cap slots themselves. `revoke(cap)` eagerly deletes all descendants, with the kernel walk made preemptible/restartable since it is unbounded. This is required, not optional: retyping untyped memory is only sound if the kernel can establish that no outstanding caps reference the region, and revoke is how exclusivity is proven. (The verified form of "no outstanding caps reference the region" is structural — *the untyped has no immediate CDT child*. That this implies the real property, *every cap into the carved physical region is a CDT descendant of this untyped*, holds **by construction**: the only frame-creation path, `retype_install`, sets `parent == Some(ut_slot)`. It is not a Store invariant, because the Store seam deliberately carries no physical-memory model — so this is a documented trusted-base bridge, §6.1 (a); D-C1.) Capabilities queued in in-flight messages occupy real, CDT-visible slots owned by the channel (§3.4), so **revocation sees through queues** — the guarantee is unconditional, with no "except messages in flight" caveat. That unconditional guarantee is *descendant-deletion* — in-flight caps included — and is reachable from the real `cap_revoke` path (`kcore::cspace::revoke` proves `first_child is None` + `cspace_wf` for any target, homed or not; D-A1, doc 70). Survival of the *revoked cap itself* across the walk is a separate axis, conditional on the target being un-homed — see §2.5's reclaim note (D-A2). Kernel caps never touch disk, so the CDT's pointer-web nature poses no persistence problem.
...
**Untyped row, who chooses the mask (Draft decision OQ-1 — to be ratified at finalization; §11).** The table lists the untyped row's rights mask as a deriver-chosen attenuation, by symmetry with the adjacent rows; the implementation does not let the caller choose it. `derive` refuses `Untyped` caps outright, and the only sub-untyped path — `retype` — pins the child's rights to a fixed `parent.rights & (READ|WRITE)` strip (∀-proven by `bit_vector` to clear `PHYS`, so physical-address authority provably never flows down an ordinary derivation chain), with page-aligned range containment proven by `carve_place`. Because READ and WRITE are the only meaningful rights on an untyped, a caller mask would have nothing to attenuate; the proposed resolution is to ratify the fixed strip and amend the table accordingly. See §11 for the alternative (D-B2).

**Thread rights bits:** `bind-reports` (configure the on-exit/on-fault slots, §5.1), `read-report` (read the terminal record; later also the debugger's register access — deferred, §8), `manage` (suspend; resume is the deferred §5.3 addition). The §5.4 maximum-controlled-priority ceiling is a value on the cap, not a bit, and attenuates the same monotone way — realized as a `u8` on the thread cap and **Verus-verified** (`kcore::cspace::derive` proves `child.max_prio ≤ parent.max_prio` ∀, exactly like the rights mask; D-B1, doc 70). The "*attenuated as desired*" below is now literal: `derive` takes a `prio_ceiling` and proves `child.max_prio == min(parent.max_prio, prio_ceiling)`, so a supervisor handing out a thread cap (the `cap_copy` ceiling argument, `cap_copy_prio`) can grant a **strictly lower** ceiling, not just preserve the parent's (D-B1 Option 2, doc 71). Kill is deliberately not on the list: destruction is not a thread operation but a property of resource ancestry (§2.2) — a supervisor holding an attenuated thread cap can observe and suspend, while only the funder, sitting above the thread's memory in the CDT, can destroy. Handing a child's main thread cap to a third party, attenuated as desired, *is* the supervision grant — revocable like any other cap.
...
**Mapping state lives in the frame cap: one mapping per cap copy, and deleting or revoking the cap unmaps it.** This single rule makes shared memory obey the same revocation story as everything else. The bulk-IPC path (§3.1) is: derive a frame cap copy (attenuated to read-only if desired), send it (§3.4), receiver maps it — and revoking the parent cap unmaps every sharer everywhere, with no special machinery. (Verified boundary, §6.1 (c): the cap-side bookkeeping is proven — a derived copy starts unmapped, and `delete` drives `aspace_unmap` with the cap's own recorded coordinates — but `aspace_unmap`'s contract frames object/cspace state only, saying nothing about page-table entries. The actual PTE clearing is proven separately in `unmap_in` over raw page-table slices (`pt_lookup → None`, one TLBI per page); the join — that the cap's recorded mapping *is* the true PTE location and that `aspace_unmap` truly clears it — lives in the unverified kernel store. So "deleting the cap unmaps it" is proven up to a trusted host-checked seam, not end-to-end; D-C3.)

**Root-survival across `revoke`, recorded honestly (D-A2, doc 70/72).** The reclaim and grant patterns rely on the granter *retaining its parent cap* after a `revoke` empties the descendants — the §5.1 `revoke(untyped)` → `reset` reuse needs the untyped cap to survive, and a server reclaiming a session window keeps the window's parent. `revoke` only ever deletes *descendants* of its target, so the target itself can empty only through the cross-object teardown that fires when a revoked descendant holds the **last** capability to an object that *homes* the target (a cspace whose resident, a channel whose ring slot, or a TCB whose bind slot *is* that cap). Survival is therefore Verus-verified for an **un-homed** root — a top-level / donated-untyped cap that is no object's home handle (`kcore::cspace::revoke` ensures `¬is_homed(slot) ⟹ the cap survives`) — and the only admitted corner is the self-referential **seL4-zombie**: a homed root whose own subtree holds its homing object's *last* cap, self-emptied by the cascade (witnessed by `revoke_can_empty_its_own_root_zombie`; the un-homed branch by `check_revoke_root_survives`). The donation/reuse patterns never build that shape — a reclaimable grant always keeps a reference to the funder *outside* the granted subtree — so survival covers them. That intermediate *resident-with-external-reference* case (a homed root whose homing object keeps a live reference outside the revoked subtree) is now **Verus-verified** too: `revoke` ensures `is_empty_cap(final[slot]) ⟹ ∃o. o homes slot ∧ o was destroyed` (the refs-monotone `emptied_via_dead_home` frame; D-A2, doc 72), so a caller that keeps any homing object alive — exactly what the reclaim flow does, the granter's cspace outliving the revoked subtree — concludes the root survives by contraposition. Only the self-referential seL4-zombie, where the homing object's *last* reference is itself inside the revoked subtree, self-empties.
...
**Monotonic time** comes from the ARM generic timer: CNTVCT is readable from EL0, giving every process a zero-syscall monotonic clock; the kernel timer object (§3.6) programs deadline interrupts for timeouts and the userspace flush timer (§4.4). (MVP resolution boundary: the deadline→signal coupling is Verus-proven — `arm` binds deadline + bits + notification, `check_expired` fires `signal` once `deadline <= now` — but expiry is evaluated on the periodic scheduler tick (§5.4), so MVP deadline resolution is **one tick**, not the exact `CNTV` compare register. The `CNTVCT`/`CNTV` access and tick programming are unverified kernel-crate concerns, and the spec does not quantify timing resolution. D-E4.)
...
- Channels expose **readability** and **writability** notifications ("signal me when a message / space appears") and a **peer-closed** notification (raised when the other endpoint is destroyed — required for session cleanup, §2.4). **Teardown always signals:** deleting one endpoint fires the surviving peer's binding, and destroying the whole object at once — its backing untyped revoked, the normal case when a session's funder dies (§3.5) — fires every endpoint's binding before reclamation; the bound notification is a separate object and outlives the channel if separately funded. (Firing locus, recorded honestly: `destroy_channel` does not itself fire peer-closed — the surviving peer is fired by per-endpoint-cap `delete` (`endpoint_cap_dropped`) as the revoke walk deletes the endpoint caps, *before* `destroy_channel` reclaims the now-empty object. The net behavior the prose promises is delivered and machine-proven through the refcount chain — `obj_refs == 0 ⇒ both endpoint caps already deleted, each having fired its peer with bindings intact` — for the only spec-admitted whole-object destruction path, untyped revoke as a CDT walk over the endpoint-cap descendants; the locus simply differs from the prose. D-D2.) Afterward a dead endpoint cap yields error returns. Queue memory is touched only through syscalls, so channel death is always delivered as an event, never as a fault.
...
A cap leaves the sender's cspace at send time, occupies a slot in the message while queued, and lands in the receiver's cspace at receive time. **At every instant a capability has exactly one owner** — sender, queue slot, or receiver, never two. A sender that wants to keep access duplicates first: a deliberate, auditable act. **Verus-witnessed (D-D1, doc 69):** `kcore::channel::recv` now exports the *receive* half of this move, restoring symmetry with the send half (`send` already proved the source slots emptied and the cap pushed onto the queue). On a successful receive its `ensures` give: for every non-empty arriving cap, the destination slot the caller named holds *that* cap afterward (`final.slot_view()[dests[c]].cap == <arriving cap>`); every cap slot of the dequeued head is empty; and the returned install mask names exactly the filled destinations (bit `c` set iff arriving cap `c` was non-empty). The installation was always performed in the body (`slot_move` into each `dests[c]`) but was previously absent from the contract — a verified caller could conclude only that the cap *left* the queue, not where it *went*. Together with the send-side export and the revoke-side `no_live_descendant` (D-A3), the "exactly one owner" invariant is now a Verus `ensures` at each transfer point, not merely an effect of the body. The real-op test `recv_installs_exact_caps_and_mask` drives a two-cap message through the *real* `recv` and checks the caps land by value with mask `0b101`.

Consequences, all load-bearing:

- **Queue slots are real, CDT-visible capability slots** owned by the channel (allocated from the memory donated at creation). Revocation therefore finds and deletes in-flight caps like any other descendants — no special case in the revoke logic, no caveat in its specification (§2.2, §6). **Verus-witnessed (D-A3, doc 73):** `kcore::cspace::revoke` now exports `no_live_descendant(final, slot)` — afterward no live slot, in-flight ring cap included, is a CDT descendant of the target — discharged from `first_child is None` + `cspace_wf` by `lemma_childless_no_descendant`, with the real-op test `revoke_sees_through_queued_descendant` driving a genuine channel ring cap to empty through the *real* `revoke`. This transitive reach was previously only structurally implied (no failing obligation if `cdt_wf`/`slot_move` drifted); a queued cap is a descendant because `slot_move` (what `send` uses) inherits the parent edge into the ring slot. The fully ∀-quantified "every cap that was a descendant in the initial state is empty in the final state" remains a follow-on, entangled with the cross-object teardown cascade.
...
Read both superblock slots; discard any failing checksum (a torn superblock write can only damage the slot being written — the other is a complete older commit); take the survivor with the higher generation. Its ref table defines reality. **(Draft decision OQ-3 — to be ratified at finalization; §11.)** "Higher generation" leaves one case open in the forged-bytes regime the next sentence raises: two checksum-valid slots with *equal* generation — impossible for honest commits (the protocol's `GenerationsDistinct`), but constructible by a forger. `pick_survivor` resolves it deterministically to **slot A** (a total `>=` tie-break, never a refusal), matching the TLA+ `LiveSlot` `>=` rule. This is safe because the survivor — whichever slot — is then run through the Verus-total `validate_geometry_fields` before any field sizes an allocation or read, so a forged tie can neither panic, over-read, nor unbounded-alloc; it is no worse than the single-forged-slot case this section already concedes. The proposed resolution is to ratify slot-A-on-tie; the alternative (refuse to mount on an equal-generation tie) is in §11 (D-G2). **The checksum detects torn writes; it authenticates nothing** — there is no secret in it, and a checksum-valid superblock proves a complete write, not a write by this system. Mount therefore validates every geometry field of the surviving superblock — index and free-list locations, WAL region, chunk tail — against the one ground truth it holds, the actual device length, with checked arithmetic, at a single chokepoint immediately after checksum verification; only past that point may downstream code size allocations or reads from superblock fields. (Untrusted fields must never vouch for each other: a length gated only by `chunk_tail` is untrusted data validated against untrusted data.) Replay the WAL from the recorded head to rebuild per-ref overlay state for acknowledged-but-unflushed writes; discard checksum-failing tail records (never acknowledged, so safe). Unreferenced chunks from interrupted flushes are invisible and reclaimed by the next GC.
...
**There is no kernel process object; the thread is the unit of report.** A "process" is a loader/runtime convention — an aspace, a cspace, a main thread, and the untyped that funds them — and the kernel's share of its lifecycle lives entirely in the thread object. Each TCB carries two fixed binding slots, configured by the holder of the thread cap exactly as channel endpoints are (§3.6): **on-exit** and **on-fault**, each a (notification cap, bit) pair, CDT-visible like queue slots so revocation sees through them. Alongside sits a preallocated **terminal report record** — `running | exited(status word) | faulted(cause, faulting address)` — at most one terminal report per thread (suspend-on-fault means no second fault), so event delivery never allocates (§3.6). Two syscalls complete the design: `thread_exit(status)`, the only voluntary stop, recorded by the kernel so a child can neither lie about nor forget its own death; and `read_report(thread cap)`. (Verified boundary, §6.1 (d): the load-bearing report properties *are* mechanized in kcore — `report_terminal` proves first-write-wins/absorbing, `set_tcb_report` is the sole `.report` mutator and runs only from `report_terminal`, and `destroy_tcb` proves the report survives destruction, i.e. persistence. What is trusted shell is only the thin `thread_exit`/`read_report` syscall dispatch and register marshalling, placed outside the verified core by §6's tiering. D-F4.) Spawn therefore returns a **process record**, plain data whose kernel-visible heart is the main thread cap; process status is the main thread's status, by convention. The canonical parent loop: bind on-exit/on-fault to a notification, start the thread, wait, `read_report`, revoke the donated untyped — whole-child teardown by resource ancestry (§2.2); kill was never a thread operation — and reuse the slots. Because bindings are configured by the cap holder and a child holds no cap to its own threads, a child cannot silence or forge its own death notice. (Verified boundary, §6.1 (d): kcore's `bind` proves only the *mechanical* binding edit. The security predicate — that only the thread-cap holder may invoke it, and that the dying thread cannot — is enforced by the unverified `BIND_REPORTS`/`READ_REPORT` rights gates in the syscall layer plus the spawn-time cap-distribution convention, not by a kcore `ensures`: Store-seam functions operate on `ObjId`s and cannot see caller authority. The `Rights` bits themselves and `derive`'s rights-monotonicity *are* in the verified model; the binding of a specific right to a syscall entry, and the cap-distribution structure, are not. D-F3.) Orphans do not exist, because there is no process lifetime independent of funding: parent death revokes the parent's untyped and the entire descendant tree with it; "adoption" is a supervisor having been granted thread caps before the parent died, and a binding whose notification has died is a slot the revoke already cleared — signaling nothing is a no-op. Exit status persists in the TCB until the parent reclaims the thread, and the TCB is parent-funded memory: the reaping incentive is the parent's own budget, and there is no global zombie table to leak.
...
|---|---|---|---|
| Protocol models | **TLA+** | (a) storage commit/recovery protocol, (b) kernel cap revocation | **Before implementation.** (a) State = (slot A, slot B, chunk-store set, WAL, per-ref flushed/unflushed status); the model covers **partial flushes** — one ref's new root committed while another's overlay is unflushed — with invariant: *after any crash, recovered state = committed roots + replay of all WAL records not covered by the committed head*. This protocol **content** invariant stays TLA+-owned; since the Verus port `cas` additionally carries **Verus** structural proofs over the real recovery code — WAL framing (`decode_frame`), maximal-run replay coverage (`replay_bound`, with `lemma_gap_freedom` proving every unflushed record lies in the replayed span — the code shadow of `AckedWritesRecoverable`'s WAL-replay half), and A/B slot selection (`commit_target` writes the non-live slot, the by-construction witness of `AtLeastOneValidSlot`; `pick_survivor` matches the `LiveSlot` ≥ tie-break) — i.e. the *in-bounds / totality / maximal-run* half. Per-record content acceptance is left uninterpreted there (`content_ok_spec`) and `commit()` is plain Rust over those verified pure decisions, so the full replay-equality invariant is mechanized nowhere on the real code — it remains TLA+'s alone. (b) The revocation model **includes channel queue slots**, so "revoke destroys all descendants" is checked unconditionally, in-flight caps included. |
| Proof-carrying code | **Verus** | `kcore` (cspace/CDT, untyped retype, channels, notifications, timers, thread reports, the page-table walker, syscall decode) plus the host-side chokepoints `ipc`, `urt`, `dma-pool`, `cas` | **Completed (`doc/plans/3_verus-rewrite.md`).** The original plan to write these in Verus dialect "from day one" slipped — the kernel predated the tooling and was first mechanized under Kani (below) — but the Verus port was then carried out: `kcore`/`ipc`/`urt`/`dma-pool`/`cas` now verify **unbounded** under `cargo verus verify` in CI (job `verus`; Verus 0.2026.06.07.cd03505, toolchain 1.95.0). The rewrite's shape — a host-buildable `kcore`, explicit `wf()` predicates, the `Env`/`Hal` hardware seam, no integer→pointer casts in the core — is exactly what the port needed. |
| Bounded model checking | ~~**Kani**~~ — retired | (formerly `kcore` + the host-side chokepoints `ipc`/`urt`/`dma-pool`/`cas`) | **Retired as of phase 7f (`doc/plans/3_verus-rewrite.md`, `doc/results/62`).** Kani was the interim bounded mechanized tier (pinned cargo-kani 0.67.0, CI job `kani`); it found and fixed real defects (carve overflow; an executable-MMIO encoding), recorded in `doc/results/2_kani-findings.md` … `8_kani-findings-7.md`. Every target it covered is now proven **unbounded** in Verus (above), so no `#[kani::proof]` harnesses or `kani` CI job remain. |
...
Where Verus proves a property it proves it well — rights-mask monotonicity is ∀-quantified, not sampled; the `revoke` walk structurally forces *every* transitive descendant gone; the channel FIFO and the notification waiter queue are faithful deterministic encodings; mount geometry validation is total over arbitrary bytes. But several spec safety properties are delivered only **up to honest trusted-base seams** — the same discipline §4.8 states for "fsync means fsync," generalized and collected here once so no downstream artifact mistakes a property routed to trust for a mechanized one. Each below is established by construction or in the unverified shell, not by a Store `ensures`:

- **(a) Physical-region exclusivity (D-C1).** "No outstanding caps reference the region" is operationalized as "the untyped has no immediate CDT child." That every cap into the carved physical region is a CDT descendant of its untyped holds *by construction* — the only frame-creation path, `retype_install`, sets `parent == Some(ut_slot)` — not as a Store invariant, since the Store seam carries no physical-memory model. (§2.2.)
- **(b) Cross-root untyped non-overlap (D-C2).** In-untyped disjointness is proven (watermark monotonicity), and sub-untyped/sibling disjointness follows from parent containment (`carve_place`). The disjointness of the *independent root untypeds* — slot 0, slot 2, the device frame, the aspace pool — set up in the boot shell is a **boot-setup axiom**: the static `base`/`size` literals live in `unsafe` code outside `verus!`, with no global frame table, so their non-overlap and the int→ptr seam are trusted; `carve_place` + watermark monotonicity then propagate disjointness within each root. (§2.5.)
- **(c) cap↔PTE correspondence and actual PTE clearing (D-C3).** `aspace_unmap`'s contract frames object/cspace state only; the real PTE clearing is proven separately in `unmap_in` over raw page-table slices (`pt_lookup → None`, one TLBI per page). The join — the cap's recorded mapping *is* the true PTE location, and `aspace_unmap` truly clears it — lives in the unverified kernel store. (§2.5.)
- **(d) Thread-lifecycle shell (D-F1, D-F3, D-F4).** kcore mechanizes the report record-mutation (`report_terminal`, first-write-wins/absorbing), its sole mutator (`set_tcb_report`), and report-survives-destroy (`destroy_tcb`). It does *not* mechanize: the "suspended, never rescheduled" state (exception entry / syscall exit / scheduler, D-F1); the anti-forgery / anti-suppression access control (the `BIND_REPORTS`/`READ_REPORT` rights gates and the spawn-time cap-distribution convention, D-F3); or the `thread_exit`/`read_report` syscall dispatch and register marshalling (D-F4). (§5.1, §5.3.)
- **(e) Storage-recovery content coverage (D-G1).** Already stated in row (a): the Verus surface over the WAL/superblock mechanizes the in-bounds / totality / maximal-run *structural* half; per-record content acceptance is uninterpreted (`content_ok_spec`) and `commit()` is plain Rust, so the full replay-equality invariant is mechanized **nowhere on the real code** — it remains TLA+'s alone. Any artifact reading §6 row (a) as "fully Verus-mechanized" is wrong.

This list was once longer: the §5.4 priority axis was the one place "monotone like every other derivation" was prose, not proof. That seam is now closed — the cap-carried `max_prio` and `derive`'s monotone attenuation are Verus-verified (D-B1, §2.3/§5.4) — and is no longer trusted.
...
## 11. Open questions for rev3 finalization (draft)

These are the **only** unsettled decisions in this draft. Each is a point where the rev2 prose
was genuinely silent and the implementation committed to a reading; the drift audit
(`doc/results/69_verus-findings.md`) surfaced them as accepted-documented. Everything else in
this document is either settled rev2 text or a faithful description of what the verified code
already does — including the trusted-base seams of §6.1, which are documented boundaries, not
choices to make. For each question below the **current code reading** is given, then a
**proposed resolution** (recommended, *not yet ratified*) and the **alternative**. Resolving
these three — by ratifying the proposal or choosing the alternative — is what moves rev3 out
of draft. Inline markers at §2.3, §3.6, and §4.5 point here.

**OQ-1 — Who authors the untyped row's rights mask? (D-B2; §2.3.)**
The §2.3 lattice lists "Untyped / memory | sub-range (page-aligned) + rights mask," reading by
symmetry as a deriver-chosen mask. *Current reading:* the caller does not choose — `derive`
refuses `Untyped`, and `retype` pins the child to a fixed `parent.rights & (READ|WRITE)` strip,
∀-proven by `bit_vector` to clear `PHYS`. *Proposed resolution:* ratify the fixed strip and
amend the table to say so, since READ and WRITE are the only meaningful untyped rights, leaving
a caller mask nothing to attenuate. *Alternative:* add a caller-supplied mask argument to
`retype` for literal table symmetry — more surface, no new expressible authority, so not
recommended.

**OQ-2 — Does a zero-result signal wake a queued waiter? (D-E3; §3.6.)**
§3.6 says "signalers OR bits in; a waiter receives the accumulated word, which clears," and is
silent on a signal whose post-OR word is still zero while a waiter is queued. *Current reading:*
treated as an accumulate, not a wake — no information is conveyed (delivering a zero word is
vacuous), and the IPC reactor only ever signals a nonzero single-bit mask. *Proposed
resolution:* ratify the no-wake reading and state it in §3.6. *Alternative:* define a
zero-result signal as a spurious wake (waiter returns with word 0) — costs nothing in safety
but adds a wake with no event behind it, contrary to the poll-once-then-wait discipline, so not
recommended.

**OQ-3 — How is an equal-generation superblock tie broken? (D-G2; §4.5.)**
§4.5 says recovery takes "the survivor with the higher generation"; honest commits keep
generations distinct (the protocol's `GenerationsDistinct`), but a forger can present two
checksum-valid slots with equal generation. *Current reading:* `pick_survivor` is total and
resolves an equal-generation tie deterministically to **slot A** (a `>=` tie-break, never a
refusal), matching the TLA+ `LiveSlot` rule; whichever slot wins is then run through the
Verus-total `validate_geometry_fields` before any field sizes an allocation or read, so a forged
tie cannot panic, over-read, or unbounded-alloc. *Proposed resolution:* ratify slot-A-on-tie and
note in §4.5 that it is safe because the survivor is fully geometry-validated before use — no
worse than the single-forged-slot case §4.5 already concedes. *Alternative:* refuse to mount on
an equal-generation tie — a harder failure stance with no safety gain given the downstream
validation, so not recommended.

**Not an open question — a tracked verification residual.** The fully ∀-quantified
sees-through-queues theorem ("*every* cap that was a CDT descendant of the target in the initial
state is empty afterward") remains a follow-on, entangled with the cross-object teardown
cascade; the named obligation `no_live_descendant` plus a real-op test already discharge the
load-bearing M1 case (§3.4, D-A3). This is a proof-completeness item, not a spec decision, and
needs no resolution to leave draft.