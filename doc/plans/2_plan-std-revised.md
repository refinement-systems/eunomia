...
| **thread: spawn/join/yield/sleep** | `Retype→Thread` + `ThreadStart(13/18)`; `ThreadExit(15)`; join via `ThreadBind(21,on-exit)`+`NotifWait(12)`+`ReadReport(22)`; **`Yield`=op 2**; sleep `TimerArm(14)`+`NotifWait` | **3.2** | new `urt` in-process thread primitive (rename to avoid colliding with the existing process-spawn `urt::spawn`); the closure crosses `x0` as a **thin** `Box<ThreadInit>` (see 3.2); zero Verus/host coverage today |
...
| **thread_local / TLS** | `TPIDR_EL0` (absent) + `urt::slots` (verified) | **3.1 / 3.5** | `TPIDR_EL0` save/restore (3.1) + `urt` TLS key table (3.5) + `sys/thread_local` arm |
...
The verification posture is `kernel/`-over-`kcore` throughout. **Net remaining
kernel-track surface: one change (`TPIDR_EL0`)** — the entropy MVP adds no kernel work.
...
| 2 | `TPIDR_EL0` save/restore | trusted-kernel-shell | `kcore/src/thread.rs` `TrapFrame` (outside `verus!{}`) + `kernel/src/exceptions.rs`, `main.rs`, `syscall.rs` | trusted asm shell (rev2§6.1d); ledger *routing note*, no new seam | **Yes — the one genuine kernel-track touch (3.1)** |
...
| **3.2** | spawn/join/yield/sleep **+ heap lock** | **(a) Heap lock first — the concurrent-alloc fix.** `thread::spawn` opens simultaneous allocation against the process-global `urt::Heap` (`unsafe impl Sync`, no lock today) — the boxed closure, per-thread TLS blocks (3.5), and any user/std allocation on the child race the parent. Introduce a raw **yielding spinlock** primitive (in `urt`/`eunomia-sys`) and wrap the `Heap` `fl` access; rewrite the `urt::Heap` `Sync` justification from "no concurrent access by construction" to "mutual exclusion by the heap spinlock", and update the `sys/alloc` + findings-5 disclosure. The lock is the correct choice, **not** per-thread arenas: cross-thread frees (a `Box` moved to another thread and dropped) can't locate the owning arena without a global structure. The lock **yields** rather than pure-spins (priority-inversion mitigation, see 3.3). **(b) Thread primitive.** New `urt` in-process thread-spawn primitive (retype-TCB + fund stack/guard per rev2§5.3 + `thread_start`) — **rename to avoid colliding with the existing process-spawn `urt::spawn`** (`SpawnRec`/`arm`/`reap`, a CDT donation subtree). The closure crosses `x0` as a **thin** pointer: `ThreadStart` carries one `u64` arg (kcore/kernel zero the rest of the frame), so pass `Box::into_raw(init).expose_provenance() as u64` where `init: Box<ThreadInit>` is already thin (the fat `Box<dyn FnOnce()>` lives *inside* the sized `ThreadInit`), and reconstruct in the trampoline — mirroring `sys/thread/motor.rs` line-for-line. `sys/thread/eunomia.rs` arm; `yield_now` = op 2; sleep = `TimerArm`+`NotifWait` | **Loom** certifying the heap spinlock's mutual exclusion (same tier as the futex bucket lock — never Verus, SeqCst pin) + QEMU spawn smoke that **forces simultaneous allocation** on ≥2 threads + **new host tests** for the `urt` thread-spawn invariants (bind-before-start, read-report-before-revoke) — currently uncovered | **9** |
...
| **3.5** | TLS keys | `urt::tls` key table over the verified `SlotAlloc` + per-thread block over the verified (now lock-guarded) heap; `sys/thread_local` arm | `cargo verus verify -p urt` (key-table obligations green); host test | **12** |
