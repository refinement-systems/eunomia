...
| **3.5** | TLS keys (hardening) | **The `local_pointer!` current-thread/id path + the `TPIDR_EL0` per-thread block + `sys/thread_local/eunomia.rs` shipped early in 3.2** (findings #9 — `set_current` needs per-thread storage the moment `spawn` exists). 3.5 is now the *hardening* remainder: the **verified `urt::tls` key table** over `SlotAlloc` (replacing 3.2's runtime slot-counter), TLS **destructors** (drop-current on thread exit — 3.2 leaks the block), and the `thread_local!`-macro storage (currently single-threaded `no_threads`; needed for `HashMap`'s `RandomState`, 3.4) | **DONE (#12):** verified `urt::tls` `KeyTable` over `SlotAlloc` (urt 25→29); eunomia moved onto std's key-based `os.rs` storage (the motor shape via the `__eunomia_tls_*` bridge), replacing the raw `NEXT_SLOT` counter; destructors + block-free driven at the trampoline/`_start` exit (the hermit posture). `cargo verus verify -p urt` **29/0** + `-p eunomia-sys` 7/0; urt Miri 46; QEMU `stdsmoke tls` → **`STD35 PASS`** | **12** |
