...
fn main() {
    ...
    let reqs: &[(&str, Request)] = &[
        ("read", Request::Read { handle: 0, path: path(&[b"etc", b"conf"]), offset: 0, len: 64 }),
        ("write", Request::Write { handle: 0, path: path(&[b"etc", b"conf"]), offset: 0, data: b"hi".to_vec() }),
        ("unlink", Request::Unlink { handle: 0, path: path(&[b"etc", b"conf"]) }),
        ("list", Request::List { handle: 0, path: path(&[b"etc"]) }),
        ("open_child", Request::OpenChild { handle: 0, path: path(&[b"etc"]), rights_mask: 0xFF }),
        ("close", Request::Close { handle: 0 }),
        ("sync", Request::Sync { handle: 0 }),
        ("snapshot", Request::Snapshot { handle: 0, message: b"m".to_vec(), class: 1 }),
        ("list_snapshots", Request::ListSnapshots { handle: 0 }),
        ("open_snapshot", Request::OpenSnapshot { handle: 0, snap_id: 1, path: path(&[]), rights_mask: 0xFF }),
        ("rollback", Request::Rollback { handle: 0, snap_id: 1 }),
        ("revoke_ref", Request::RevokeRef { handle: 0 }),
        ("mint_ticket", Request::MintTicket { handle: 0, ttl_nanos: 1_000_000 }),
        ("redeem_ticket", Request::RedeemTicket { ticket: [0xAB; 16] }),
        ("stat", Request::Stat { handle: 0, path: path(&[b"etc", b"conf"]) }),
        ("enumerate", Request::EnumerateSession),
        ("delete_snapshot", Request::DeleteSnapshot { handle: 0, snap_id: 1 }),
        ("set_class", Request::SetClass { handle: 0, snap_id: 1, class: 0 }),
        ...
        ("read_subtree_h1", Request::Read { handle: 1, path: path(&[b"conf"]), offset: 0, len: 64 }),
        ("list_snapshot_h2", Request::List { handle: 2, path: path(&[]) }),
    ];
    ...
    write_seed("structured_request", "mixed", b"\x01\x00\x05etc\x04conf\x00\x02hi");
