fn main() {
    ...
    // the bare-metal target (`*-none` or the `aarch64-unknown-eunomia` os), and
    // scope to bin targets so a host test harness never receives them. (Without
    // this the console links at the default 0x200000 instead of the rev2§5
    // process base 0x80000000, so `spawn::prepare` maps it at the wrong VA.)
