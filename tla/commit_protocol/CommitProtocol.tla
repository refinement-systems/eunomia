...
\* Storage commit / crash-recovery protocol for Eunomia OS (spec §4.2–4.5, §6).
\*
\* Abstraction: writes to a ref are numbered 1,2,3,… per ref ("versions").
\* Content is last-write-wins (§4.4), so the tree flushed at version v
...
\*
\* Modeling decisions:
\*   - Write collapses memtable-write + WAL-append + WAL-fsync (§4.3
...
\*     torn = checksum-invalid, discarded by recovery (§4.5).
...
\*     no superblock, invisible to recovery, reclaimed by GC (§4.5) —
...
\*
\* Headline invariant (spec §6): after any crash, recovered state =
...
\* The live superblock: valid slot with the higher generation (§4.5).
LiveSlot ==
...
\* Nothing on disk references the result yet (§4.3 step 3).
Flush(r) ==
    /\ ~crashed
    /\ commitPhase = "idle"
    /\ overlay[r] /= {}
...
\* record (§4.3 step 4, §4.4). A commit may carry any subset of refs
...
\* Only after this is the commit real (§4.3 step 4).
CommitFinish ==
    /\ ~crashed
    /\ commitPhase = "prepared"
    /\ IF OlderIsA
...
\* being written, never the other one (§4.5).
Crash ==
    /\ ~crashed
    /\ crashed' = TRUE
...
\* Recovery (§4.5): the live slot defines reality; rebuild each ref's
...
\* §4.5: a torn superblock write can only damage the slot being written;
...
\* Barrier 1 (§4.3): no superblock — not even a stale-but-valid one —
...
\* The headline recovery invariant (§6): every acknowledged write is
