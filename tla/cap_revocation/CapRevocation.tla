...
\* SYMMETRY collapses each permutation-orbit of interchangeable model values to a
\* single representative: a sound state-space quotient that checks the same
\* behaviours in fewer states. Sound ONLY when no temporal property is checked —
\* TLC's symmetry reduction is unsound under liveness — so these operators are
\* named by the invariant-only CapRevocation_Safety.cfg (and its symmetric
\* negative controls), NEVER by the liveness CapRevocation.cfg. TLC never
\* validates a symmetry itself; the committed symmetric negative controls run
\* under the same SYMMETRY are the standing proof each quotient has teeth.
\*
\* The processes in Procs are interchangeable: every action and invariant treats
\* them uniformly, and the only initial asymmetry — the arbitrary CHOOSE that
\* picks which process holds the root cap (InitProc) — is itself symmetric, since
\* any process serves equally.
ProcSymmetry == Permutations(Procs)

\* The cap slot IDs in CapIds are likewise interchangeable: every action and
\* invariant quantifies over them uniformly and none hard-codes a specific id.
\* parent maps CapIds to CapIds, so a cap permutation carries a CDT forest to an
\* isomorphic one. The only initial asymmetry is the InitCap == CHOOSE c \in CapIds
\* seed (Init gives the root cap to exactly InitCap) — the structurally-risky
\* asymmetric-CHOOSE-over-a-symmetric-set case — but the choice is itself
\* symmetric (any cap serves equally) and TLC canonicalises the CHOOSE
\* consistently. The cap-singling SpecCapAsymBad negative control, run under this
\* same SYMMETRY, is the standing probe that the cap quotient is not over-broad
\* against that seed (the dual of the process-singling SpecAsymBad).
CapSymmetry == Permutations(CapIds)

\* The TCB ids in Threads are interchangeable too: every action quantifies over
\* them uniformly (Bind/ThreadExit/ThreadFault, and DeleteOne rewrites every
\* thread's slots at once), and Init seeds them identically — all "running", all
\* binding slots NULL — with NO CHOOSE over Threads, so this axis carries none of
\* the asymmetric-seed risk the InitProc/InitCap quotients do. The two
\* thread-ranging properties the safety arm checks under the quotient — the
\* FireSafe invariant and the ReportMonotone action property — are
\* \A t \in Threads universals with no asymmetric thread reference, hence
\* invariant under any permutation of Threads, so the quotient stays sound for
\* both (the action property included; rev2§5.1). The thread-singling
\* SpecThreadAsymBad negative control, run under this same SYMMETRY, is the
\* standing probe that the thread quotient is not over-broad.
ThreadSymmetry == Permutations(Threads)

\* The safety arm's symmetry permutes ALL THREE disjoint model-value sets. TLC's
\* idiom for a symmetry over disjoint model-value sets is the union of their
\* Permutations (the group generated by all of them). Named by
\* CapRevocation_Safety.cfg and its four symmetric negative controls; NEVER the
\* liveness arm.
...
\* SYMMETRY-soundness control (injected ASYMMETRIC bug, CapIds axis): the dual of
\* LeakRevokedAsym for the cap quotient. AsymCap is a fixed non-InitCap id; once it
\* is ghost-revoked (hence dead) by a real Copy-then-revoke, leak it into the root
\* process's cspace — a genuine DeadNowhere violation that singles out one CAP
\* model value, so it BREAKS the CapIds-symmetry premise and its reachable path
\* crosses states the quotient collapses. Run under SYMMETRY SafetySymmetry
\* (CapRevocation_CapAsymBug.cfg) the quotient must STILL report it: where
\* LeakRevokedAsym singles out a process and so guards Permutations(Procs), this
\* singles out a cap and so guards Permutations(CapIds) — catching an OVER-BROAD
\* cap symmetry that merged an asymmetric violating state with a non-violating one,
\* the exact risk the asymmetric InitCap CHOOSE seed carries.
AsymCap == CHOOSE c \in CapIds : c /= InitCap

LeakRevokedCapAsym ==
    /\ AsymCap \in revoked
    /\ cspaces' = [cspaces EXCEPT ![InitProc] = @ \cup {AsymCap}]
    /\ UNCHANGED <<live, parent, queues, bindings, treport, revoked, revoking,
                   nlive, ncaps, pcbind, eopen>>

NextCapAsymBad == Next \/ LeakRevokedCapAsym

SpecCapAsymBad == Init /\ [][NextCapAsymBad]_vars

\* SYMMETRY-soundness control (injected ASYMMETRIC bug, Threads axis): the thread
\* dual of LeakRevokedAsym/LeakRevokedCapAsym. AsymThread is a fixed thread id;
\* once a real revoke has produced a dead cap, leak it into that thread's "exit"
\* binding slot — a genuine FireSafe violation (a non-NULL, non-live cap in a
\* binding slot) that singles out one THREAD model value, so it BREAKS the
\* Threads-symmetry premise and its reachable path crosses states the quotient
\* collapses. The bindings[AsymThread]["exit"] = NULL guard keeps the injected
\* fact to exactly one dead cap in one otherwise-empty slot, so it trips FireSafe
\* alone and displaces no live cap. Run under SYMMETRY SafetySymmetry
\* (CapRevocation_ThreadAsymBug.cfg) the quotient must STILL report it — catching
\* an OVER-BROAD thread symmetry that merged an asymmetric violating state with a
\* non-violating one. Where LeakRevokedAsym/LeakRevokedCapAsym single out a
\* process/cap and so guard Permutations(Procs)/Permutations(CapIds), this singles
\* out a thread and so guards Permutations(Threads).
AsymThread == CHOOSE t \in Threads : TRUE

LeakRevokedThreadAsym ==
    /\ revoked /= {}
    /\ bindings[AsymThread]["exit"] = NULL
    /\ LET d == CHOOSE c \in revoked : TRUE
       IN bindings' = [bindings EXCEPT ![AsymThread]["exit"] = d]
    /\ UNCHANGED <<live, parent, cspaces, queues, treport, revoked, revoking,
                   nlive, ncaps, pcbind, eopen>>

NextThreadAsymBad == Next \/ LeakRevokedThreadAsym
